...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	<p class="news-item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
		...
		<?endforeach;?>
		<?$arFilterI = Array("IBLOCK_ID"=>GetMessage("BLOCK_ID_NUM"), "PROPERTY_JOURNAL" => $arItem["ID"]);
        $resI = CIBlockElement::GetList(Array(), $arFilterI, false, Array("nPageSize"=>10));?>
        <?=GetMessage("NUMBER_OF_ARTICLE")?>: <?=getSize(GetMessage("BLOCK_ID_NUM"), "PROPERTY_JOURNAL", $arItem['ID'])?><br>
        <?$resI = CIBlockElement::GetList(Array('ID' => 'DESC'), $arFilterI, false, Array("nPageSize"=>1), $arSelect);?>
        <?if(getSize(GetMessage("BLOCK_ID_NUM"), "PROPERTY_JOURNAL", $arItem['ID']) > 0):?>
